...
- [~] **Extract `App.tsx` concerns** — moved gamification/session update flow to `utils/gamification.ts`, hash-view sync to `hooks/useHashViewSync.ts`, and auth session lifecycle to `hooks/useAuthSession.ts`; remaining work is initialization/sync orchestration extraction.
