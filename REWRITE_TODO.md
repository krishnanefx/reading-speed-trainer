...
- [~] **Split `db.ts` monolith** — extracted shared models/types/sanitizers/checksum/import normalization into `src/utils/db/models.ts`, sync queue utilities into `src/utils/db/syncQueue.ts`, cloud push helpers into `src/utils/db/cloudSync.ts`, manual backup import/export into `src/utils/db/importExport.ts`, cloud-pull reconciliation into `src/utils/db/cloudPull.ts`, and progress state helpers into `src/utils/db/progress.ts`; final step is separating remaining local CRUD exports into `db/index.ts` + feature modules.
